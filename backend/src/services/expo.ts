...
export async function persistNotifications(userId: string, tokens: string[], message: string, ticketIds: string[]) {
  for (let i = 0; i < tokens.length; i++) {
    await Notification.create({ userId, type: 'generic', message, targetPushToken: tokens[i], status: ticketIds[i] ? 'sent' : 'queued', ticketId: ticketIds[i] });
  }
}
