...
router.get('/finance/businesses/:id/report', requireAuth, async (req: AuthedRequest, res, next) => {
  try {
    ...
    if (!gate.ok) throw new ApiError('FORBIDDEN', 'Premium required or use points', 403);
    ...
    if (gate.ok && (gate as any).consume && usePoints) await (await import('../models/User.js')).User.updateOne({ _id: req.userId }, { $inc: { points: -((gate as any).consume) } });
...
router.get('/finance/businesses/:id/export.csv', requireAuth, async (req: AuthedRequest, res, next) => {
  try {
    ...
    if (gate.ok && (gate as any).consume && usePoints) await User.updateOne({ _id: req.userId }, { $inc: { points: -((gate as any).consume) } });
